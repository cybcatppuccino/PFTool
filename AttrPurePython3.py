...
def generate_testlist(bound, ithprime):
    testlist1 = [list1[ithprime] ** i for i in range(-4, 5)] + [-list1[ithprime] ** i for i in range(-4, 5)]
    if list2[ithprime] == 1/sympy.sqrt(2):
        ...
        testlist2 = [list2[ithprime] ** i * sympy.Integer(2) ** -j for j in range(0, 3) for i in range(-j, 3)]
    if list3[ithprime] == 1/sympy.sqrt(3):
        testlist3 = [list3[ithprime] ** i for i in range(0, 5)]
    elif list3[ithprime] == 1/sympy.Integer(3):
        testlist3 = [list3[ithprime] ** i for i in range(0, 3)]
    else:
        testlist3 = [list3[ithprime] ** i * sympy.Integer(3) ** -j for j in range(0, 3) for i in range(-j, 3)]
    testlist0 = [a * b * c for a in testlist1 for b in testlist2 for c in testlist3]
    testlist = [sympy.expand(x) for x in testlist0 if bound / 3000 <= abs(PFTool.toc(x)) <= bound / 3]
    return testlist


for num in range(1, 551):
    pfo = AESZ.AESZ(num).pfo
    sol = pfo.mp_all_sol(200)
    ...
    for i in range(8):
        ...
        for z0 in testlist:
            ...
            idt = mpmath.identify(val, list4[i], tol=10**-30)
            if idt is not None:
